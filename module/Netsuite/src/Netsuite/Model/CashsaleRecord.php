...
class CashsaleRecord extends Record
{
    ...
    public function relatedTableHtml($num){
        ...
        $transactions['deposit']['name'] = 'Deposit';
        $transactions['deposit']['data'] = $this->getTable('DEPOSIT_payment',array('docnumber'=>$num),
            array('DEPOSIT_InternalId','docnumber','docdate','refnum','transactionamount','entity','paymentmethod','memo'),
            array('DEPOSIT'=>array('condition'=>'InternalId=DEPOSIT_InternalId',
                'columns'=>array('tranid'))
            ),
            0);
        ...
        foreach($transactions as $type => $t){
            foreach($t['data'] as $d) {
                ...
                foreach($d->data as $k=>$v) {
                    if($k=='InternalId' || $k=='DEPOSIT_InternalId')
